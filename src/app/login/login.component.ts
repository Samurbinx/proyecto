...
export class LoginComponent {
  ...
  constructor(private fb: FormBuilder, private _router: Router, private _cookieService: CookieService,
  ...
  loginUser(){
    ...
    if(this.loginForm.value.email == '' || this.loginForm.value.pwd == ''){       // Comprueba que se hayan rellenado ambos campos, aunque si no lo hace, el botón estará deshabilitado   
      this._sharedService.openSnackBar('Debe rellenar Usuario y Contraseña.');
      return;
    }  

    let loginEmail: string = this.loginForm.value.email;
    let loginPassword: string = this.loginForm.value.pwd;
    ...
    this._loginService.loginUser(loginEmail, loginPassword).subscribe({
      next: (result: { role: string, token: string } | {}) => {
        if ('role' in result && 'token' in result) {
          ...
          this._cookieService.set('token', result.token, undefined, '/');
          window.history.back();
        } else {
